...
// ---------- NEW THERMAL CONFIG ----------
float SAFE_LIMIT = 35.0;
float RESET_LIMIT = 33.0;
float CRITICAL_LIMIT = 50.0;
unsigned long FAULT_TIME = 5000; // 5 sec
...
// ---------- NEW LOGIC FUNCTION ----------
uint8_t getState(uint8_t nodeId, float temp) {

  NodeState &ns = nodeStates[nodeId];
  unsigned long now = millis();

  // Immediate critical fault
  if (temp >= CRITICAL_LIMIT) {
    ns.state = 2;
    ns.hotspotActive = false;
    return ns.state;
  }

  // Above safe → start hotspot timing
  if (temp > SAFE_LIMIT) {

    if (!ns.hotspotActive) {
      ns.hotspotActive = true;
      ns.startTime = now;
    }

    unsigned long duration = now - ns.startTime;

    if (duration < FAULT_TIME)
      ns.state = 1;  // TRANSIENT
    else
      ns.state = 2;  // FAULT
  }

  // Reset only when sufficiently cooled
  else if (temp < RESET_LIMIT) {
    ns.hotspotActive = false;
    ns.state = 0;
  }
...
void setup() {
  ...
  for (int i = 0; i < 4; i++) {
    nodeStates[i].hotspotActive = false;
    nodeStates[i].startTime = 0;
    nodeStates[i].state = 0;
  }
...
void loop() {
  ...
  if (radio.available()) {
    ...
    if (pkt.type == TYPE_DATA) {
      ...
      cmd.senderId = EDGE_ID;
      cmd.type     = TYPE_COMMAND;
      cmd.value    = getState(pkt.senderId, tempC);
